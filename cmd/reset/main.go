...
var generatedFileTemplate = template.Must(template.New(generatedFile).Parse(`// Code generated by reset; DO NOT EDIT.
...
type generatedTemplateData struct {
	Package    string
...
func generatePackage(pkg packageToGenerate, generatedTypes map[*types.Named]struct{}) ([]byte, error) {
	data := generatedTemplateData{
		Package:    pkg.name,
		Structures: make([]generatedStructure, 0, len(pkg.structs)),
	}
	...
	for _, structure := range pkg.structs {
		...
		for _, field := range structure.structType.Fields.List {
			...
			for _, fieldName := range fieldNames(field) {
				generateReset(&body, "x."+fieldName, fieldType, pkg.types, generatedTypes, 1)
			}
		}

		data.Structures = append(data.Structures, generatedStructure{
			Receiver: structure.receiver,
			Body:     body.String(),
		})
	}

	var output bytes.Buffer
	if err := generatedFileTemplate.Execute(&output, data); err != nil {
		return nil, fmt.Errorf("execute generated file template: %w", err)
	}
